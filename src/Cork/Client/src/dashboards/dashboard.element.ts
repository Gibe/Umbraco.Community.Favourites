...
import { CorkService, UserModel } from "../api/index.js";
...
export class ExampleDashboardElement extends UmbElementMixin(LitElement) {
  ...
  #onClickWhoAmI = async (ev: Event) => {
    ...
    const { data, error } = await CorkService.whoAmI();
  ...
  #onClickWhatsTheTimeMrWolf = async (ev: Event) => {
    ...
    const { data, error } = await CorkService.whatsTheTimeMrWolf();
  ...
  #onClickWhatsMyName = async (ev: Event) => {
    ...
    const { data, error } = await CorkService.whatsMyName();
